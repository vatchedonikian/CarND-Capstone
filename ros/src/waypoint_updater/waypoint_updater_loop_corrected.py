...
class WaypointUpdater(object):
	...
	if self.arriving:	
	    ...
	    # else can be removed with no affection in this project
	...
	if stop_line < 0:
	    ...
	    if self.arriving:
		rospy.logwarn('ARRIVING COPY')
		rospy.logwarn('self.LOOKAHEAD_WPS = %s', self.LOOKAHEAD_WPS)
		rospy.logwarn('self.waypoint_cursor = %s', self.waypoint_cursor)
		rospy.logwarn('self.num_base_waypoints = %s', self.num_base_waypoints)
		rospy.logwarn('stop_line = %s', stop_line)
		...
		if self.arriving:
		    # no loop and the car passed the destination a little bit
		    return		
	...
	if to_index >= self.num_base_waypoints:
	    # this happens only once since self.waypoint_cursor will be reset soon
	    if not LOOP:
	    ...
	    #rospy.logwarn('to_index before mod is %s', to_index)
	    to_index %= self.num_base_waypoints 
	    #rospy.logwarn('to_index after mod is %s', to_index)
    ...
    def update_from_current_waypoints_1(self, closest_id):
	'''
	If self.current_waypoints_list is not None and the next starting position
	is less than self.LOOKAHEAD_WPS, then this function is used.
	'''
